...
int main(int argc, char *argv[])
{
   ...
   for (int k = 0; k <= num_equation; k++) { offsets[k] = k * vfes.GetNDofs(); 
      std::cout << "Offsets[" << k << "] = " << offsets[k] << std::endl; }
   BlockVector u_block(offsets);

   // Momentum grid function on dfes for visualization.
   GridFunction mom(&dfes, u_block.GetData() + offsets[0]);
   ...
   for (int ti = 0; !done; )
   {
      ...
      {
         cout << "time step: " << ti << ", time: " << t << ", dt: " << dt << ", dt_real: " << dt_real <<endl;
         if (visualization)
         {
            sout << "solution\n" << mesh << mom << flush;
         }
